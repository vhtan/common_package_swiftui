...
class ReportSOSRobberCubit extends Cubit<GenericCubitState<dynamic>> {
  final SosRepository repository;
  final AddTripRepository addTripRepository;

  ReportSOSRobberCubit({
  ...
  void submitSOS(SOSRobberSubmitRequest request) async {
    emit(
      GenericCubitState.loading(),
    );
    try {
      final apiResponse = await repository.warningSOS(request);
