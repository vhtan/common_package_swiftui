...
class SOSHistoryDetailsCubit extends Cubit<GenericCubitState<dynamic>> {
  ...
  void getChattingList(String id) async {
    try {
      final list = await repository.getSOSChattingList(id);
      emit(
        ChattingListSOSSuccess(
          list: list,
  ...
  void sendMessage(SOSProcessRequest request) async {
    await repository.sosProcess(request);
    try {
      emit(
        const DidSendSOSMessageSuccess(status: Status.success),
      );
    } on DioException catch (e) {
      emit(
        GenericCubitState.failure(e.errorMessage),
